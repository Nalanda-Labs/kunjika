...
<script>
	...
	onMount(async () => {
		let response = await api.post(`unanswered-questions/`, { uat, questions_per_page });
	...
	async function prevPage() {
		...
		let response = await api.post(`unanswered-questions/`, {
			uat,
			questions_per_page,
			direction: 'back'
		});
	...
	{#each questions as { id, slug, title, tags, shown_ts, uid, username, answers, views, answer_accepted }}
		...
		<div style="width:85%;float:left;position:relative">
			...
			<span style="float:right"
				>{shown_ts}
				<a href="/users/{uid}/{username}" style="text-decoration:none; color:#4285F4;">{username}</a
				></span
			>
		</div>
